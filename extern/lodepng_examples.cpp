...
void example1(int argc, char *argv[])
{
  ...
  else
  {
    ...
    for(size_t i = 0; i < decoder.getInfoPng().itext.num; i++)
      std::cout << decoder.getInfoPng().itext.keys[i] << " (" << decoder.getInfoPng().itext.langtags[i] << " "
                << decoder.getInfoPng().itext.transkeys[i] << ") : " << decoder.getInfoPng().itext.strings[i]
                << "\n";
    if(decoder.infoPng.time_defined)
    {
      std::cout << "modification time: " << decoder.infoPng.time.year << "-" << (int)decoder.infoPng.time.month << "-"
                << (int)decoder.infoPng.time.day << " " << (int)decoder.infoPng.time.hour << ":" << (int)decoder.infoPng.time.minute
                << ":" << (int)decoder.infoPng.time.second << std::endl;
    }
...
void example7(int argc, char *argv[])
{
  ...
  for(int i = 0; i < 16; i++)
  {
    encoder.addPalette((unsigned char)(127 * (1 + sin(5 * i * 6.28318531 / 16)))
...
void example8(int argc, char *argv[])
{
  ...
  if(decoder.hasError())
  {
    std::cout << "decoder error " << decoder.getError() << std::endl;
    return;
  }
  ...
  extrachunk = LodePNG_create_chunk(&encoder.getInfoPng().unknown_chunks.data[0], &encoder.getInfoPng().unknown_chunks.datasize[0], 0, "aaAa", 0);
  extrachunk = LodePNG_create_chunk(&encoder.getInfoPng().unknown_chunks.data[1], &encoder.getInfoPng().unknown_chunks.datasize[1], 0, "bbBb", 0);
  extrachunk = LodePNG_create_chunk(&encoder.getInfoPng().unknown_chunks.data[2], &encoder.getInfoPng().unknown_chunks.datasize[2], 0, "ooOo", 0);
  ...
  if(encoder.hasError())
  {
    std::cout << "encoder error " << encoder.getError() << std::endl;
    return;
  }
